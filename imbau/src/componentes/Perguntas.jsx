...
function Item({ item, aberto, aoAbrir, indice }) {
  ...
  return (
    <Revelar as="div" atraso={indice * 60} distancia={18} className="border-t border-white/8">
      <h3>
        <button
          type="button"
          onClick={aoAbrir}
        ...
        >
          <span
            className={`text-[1.05rem] leading-snug transition-colors duration-500 sm:text-[1.15rem] ${
              aberto ? 'text-osso-100' : 'text-osso-100/75 group-hover:text-osso-100'
            }`}
          >
            {item.pergunta}
          </span>
          ...
          <span
            ...
            className={`mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full border transition-all duration-700 ease-[cubic-bezier(.16,1,.3,1)] ${
              aberto
                ? 'rotate-45 border-ouro-500 bg-ouro-500 text-noite-900'
                : 'border-white/15 text-osso-100/70 group-hover:border-white/35'
            }`}
          >
      ...
      >
        <div className="overflow-hidden">
          <p
            className={`max-w-2xl pb-7 pr-12 text-[0.92rem] leading-relaxed text-osso-100/60 transition-opacity duration-500 ${
...
export function Perguntas() {
  ...
  return (
    <section id="perguntas" className="relative bg-noite-900 py-24 sm:py-32 lg:py-40">
      <div className="mx-auto max-w-[1280px] px-5 sm:px-8">
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:gap-20">
          <div className="lg:sticky lg:top-32 lg:self-start">
            <Revelar>
              <h2 className="titulo text-[clamp(1.9rem,3.8vw,2.7rem)] leading-[1.03] text-osso-100">
                {perguntas.titulo}
              </h2>
            </Revelar>
            <Revelar atraso={110}>
              <p className="mt-5 max-w-sm text-[0.95rem] leading-relaxed text-osso-100/60">
                {perguntas.texto}
              </p>
            </Revelar>
